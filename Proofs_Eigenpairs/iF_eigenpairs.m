function F = iF_eigenpairs(x,v,theta)
%%%
...
lambda = intval(x(1));
a = intval(x(2:m+1)); %%% Fourier coefficients of the steady state
b = intval(x(m+2:2*m+1)); %%% Fourier coefficients of the eigenfunction 
k = (0:m-1)';
omega = 2*intval('pi');
mu = -k.^2*omega^2;
theta = intval(theta);
