function [F] = int_F_Fourier_Chebyshev_backward_ext(c,ta,b,h,N,n,m)
...
for k = -N:N
    ...
    F(ind+1,k+N+1) = 2*ind.*c(ind+1,k+N+1) - (-lambda_k*c(ind,k+N+1)+lambda_k*c(ind+2,k+N+1)+h*(1i)*weight.*(ta_c(ind+2,k+N+1)-ta_c(ind,k+N+1)));
end
