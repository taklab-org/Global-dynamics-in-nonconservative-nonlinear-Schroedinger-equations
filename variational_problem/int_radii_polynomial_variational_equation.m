function [r_minus,success] = int_radii_polynomial_variational_equation(C,ta,h,N,n,m,n_pad_cheb,nu)

success = 0 ;

[Fourier_tail_ta,Cheb_tail_ta] = eval_tails(reshape(ta,n,2*N+1));
% disp(['Fourier_tail = ',num2str(Fourier_tail_ta),', Cheb_tail = ',num2str(Cheb_tail_ta)])
...
% disp(['nu1 = ',num2str(nu(1)),', n_data = ',num2str(n_data),', N = ',num2str(N),', m = ',num2str(m),', n_proof = ',num2str(n)])

% disp(['total size = ',num2str(n*(2*m+1))])

% disp('Computing derivative...')
ita = intval(ta); %iangle = intval(angle); 
ih = intval(h);
iDF = int_DF_Fourier_Chebyshev(ita,ih,N,n,m);
DF = mid(iDF);
...
% disp('Computing Y0...')
Y0 = zeros(2*m+1,1); % there is a Y0 for each 2m+1 IVP
...
for j = 1:2*m+1
    ...
    F_ext = int_F_Fourier_Chebyshev_ext(c,ita,b,h,N,n,m);
...
for k=-m:m
    for ell = 1:n-1        
        ...
        weights = (nu(1).^abs(ell_2))*(nu(2).^abs(k2));
...
z_F = h*abs(T)*intval(Psi); z_F(1,:) = 2/intval(nu(1))^n; z_F = reshape(z_F,n*(2*m+1),1);
v_F = abs(iA)*z_F; 
