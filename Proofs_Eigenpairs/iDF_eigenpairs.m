function DF = iDF_eigenpairs(x,v,theta)
%%%
...
a = intval([flip(a(2:end),1);a;zeros(m-1,1)]);

Dfa_nonlinear = intval(zeros(m));
...
b = intval([flip(b(2:end),1);b;zeros(m-1,1)]);

Dga_nonlinear = intval(zeros(m));
...
Dgb = exp(1i*theta)*(diag(mu) + Dfa_nonlinear) - intval(lambda)*eye(m);
