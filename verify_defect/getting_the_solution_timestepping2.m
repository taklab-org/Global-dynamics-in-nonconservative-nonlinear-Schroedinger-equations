function [a, d_N, d_infty, a_end] = getting_the_solution_timestepping2(N,n,tspan,v_hat,rigorous,weight)
...
%   igamma = exp(1i*itheta);
  igamma = 1i;
  [~,ia2full] = convtensor(nu.*ia,nu.*ia);
  ...
  k_full = -2*N:2*N;
  nu_full =  (weight).^(-abs(k_full));
  ires_full = du_full + igamma*((4*ipi^2)*(k_full.^2).*ia_full-ia2full./nu_full);
...
%   ires_full(:,N+1:3*N+1)=0;
  d_N = mag(sum(sum(abs(ires_full(:,2*N+1)))));
  ires_full(:,2*N+1)=0;
  d_infty = mag(sum(sum(abs(ires_full).*nu_full)));
else
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
